...
function upgradeProject(projectDir) {
	...
	try {
		...
		if (detection.isNpm) {
			console.log(
				"📦 Detected npm-based project. Converting to pnpm...\n"
			);
		} else {
		...
		if (packageJson.scripts.build) {
			...
			if (
				!buildScript.includes("tsc -noEmit") &&
				buildScript.includes("esbuild.config.mjs")
			) {
				if (buildScript.match(/^node esbuild\.config\.mjs/)) {
					packageJson.scripts.build = `tsc -noEmit -skipLibCheck && ${buildScript}`;
					updated = true;
					changes.push(
						"✓ Fixed build script (added TypeScript check)"
					);
				}
			}
		}
		...
		if (existsSync(lintWrapperPath)) {
			...
			if (
				currentContent.includes("usePnpm") &&
				currentContent.includes("execSync")
			) {
				console.log(
					"✓ lint-wrapper.mjs already uses flexible detection.\n"
				);
			} else {
				writeFileSync(lintWrapperPath, FLEXIBLE_LINT_WRAPPER, "utf8");
				console.log("✅ lint-wrapper.mjs upgraded!");
				console.log(
					"   Now uses flexible pnpm/npx detection (no npm warnings)\n"
				);
			}
		} else {
