...
function migrateEslintrc(eslintrcPath) {
	try {
		...
		console.log(
			"✓ Found .eslintrc file - migrating rules to flat config format"
		);
		...
		if (error instanceof SyntaxError) {
			console.log(
				"⚠ Warning: .eslintrc file is not valid JSON, using default rules"
			);
		} else {
			console.log(
				"⚠ Warning: Could not read .eslintrc file, using default rules"
			);
		}
		return {};
	}
...
function fixBuiltinModules(esbuildConfigPath, projectRoot) {
	...
	try {
		...
		if (
			...
			updated = true;
			console.log(
				"✓ Updated esbuild.config.mjs: replaced builtin-modules with module.builtinModules"
			);
		}

		// Check if it uses ...builtins (the spread)
		if (
			content.includes("...builtins") &&
			!content.includes("...builtinModules")
		) {
			content = content.replace(/\.\.\.builtins/g, "...builtinModules");
			updated = true;
			console.log(
				"✓ Updated esbuild.config.mjs: replaced builtins with builtinModules"
			);
		}

		// Add existsSync import if not present and update entryPoints to detect main.ts location
		const hasExistsSyncImport =
		...
		const hasHardcodedEntryPoint =
			/entryPoints:\s*\[["'](src\/)?main\.ts["']\]/.test(content);
		const hasEntryPointVar = /const\s+entryPoint\s*=/.test(content);
		const entryPointIndex = hasEntryPointVar
			? content.indexOf("const entryPoint")
			: -1;
		const esbuildContextIndex = content.indexOf("esbuild.context");
		const hasCorrectEntryPoint =
			hasEntryPointVar &&
			entryPointIndex >= 0 &&
			esbuildContextIndex >= 0 &&
			entryPointIndex < esbuildContextIndex;
		const hasEntryPointDetection =
			hasCorrectEntryPoint ||
			(hasEntryPointVar &&
				content.includes('existsSync("src/main.ts")')) ||
			content.includes("existsSync('src/main.ts')");
		...
		const hasAdvancedEntryPointDetection =
			/const\s+hasSrcMain\s*=/.test(content) &&
		...
		const hasSimplePattern =
			/const\s+entryPoint\s*=\s*existsSync\(["']src\/main\.ts["']\)\s*\?/.test(
				content
		...
		if (
			...
			content = content.replace(
				/^\s*const\s+entryPoint\s*=.*?;\s*$/gm,
				""
			);
			// Also remove any that might be on the same line as other code (less common)
			content = content.replace(
				/\s*const\s+entryPoint\s*=\s*existsSync\([^)]+\)\s*\?[^;]+;\s*/g,
				""
			);
			content = content.replace(
				/\s*const\s+entryPoint\s*=\s*[^;]+;\s*/g,
				""
			);
		...
		if (hasAdvancedEntryPointDetection) {
			...
			if (hasHardcodedEntryPoint) {
				...
				updated = true;
				console.log(
					"✓ Updated esbuild.config.mjs: fixed entryPoints to use entryPoint variable"
				);
			}
			...
			const simplePatternRegex =
				/(\/\/\s*Detect\s+entry\s+point[^\n]*\n\s*)?const\s+entryPoint\s*=\s*existsSync\(["']src\/main\.ts["']\)\s*\?[^;]+;/;
			if (simplePatternRegex.test(content)) {
				// Find where to insert (before esbuild.context)
				const esbuildContextMatch = content.match(
					/(\s*)(const\s+\w+\s*=\s*(?:await\s+)?esbuild\.context\s*\()/
				);
...
`;
					...
					updated = true;
					console.log(
						"✓ Upgraded esbuild.config.mjs: enhanced entry point detection with warnings and error handling"
					);
				}
			}
		} else if (
			hasHardcodedEntryPoint ||
			(hasEntryPointVar && !hasCorrectEntryPoint)
		) {
			// Add existsSync import if needed
			if (needsExistsSync) {
				// Find the import statement and add existsSync to it, or add a new import
				if (
					content.includes('import { builtinModules } from "module"')
				) {
					content = content.replace(
					...
					const processImportMatch = content.match(
				...
				updated = true;
				console.log(
					"✓ Updated esbuild.config.mjs: added existsSync import"
				);
			}
			...
			const entryPointPattern =
				/entryPoints:\s*\[["'](src\/)?main\.ts["']\]/;
			const currentHasEntryPointVar = /const\s+entryPoint\s*=/.test(
				content
			);
			const currentEntryPointIndex = currentHasEntryPointVar
				? content.indexOf("const entryPoint")
				: -1;
			const currentEsbuildContextIndex =
				content.indexOf("esbuild.context");
			const currentHasCorrectEntryPoint =
				currentHasEntryPointVar &&
			...
			if (
				entryPointPattern.test(content) ||
				!currentHasCorrectEntryPoint
			) {
				...
				if (!currentHasCorrectEntryPoint) {
					...
					if (prodMatch) {
						...
						for (let i = 0; i < lines.length; i++) {
							if (
								/^\s*const\s+\w+\s*=\s*(?:await\s+)?esbuild\.context\s*\(/.test(
									lines[i]
								)
							) {
				...
				updated = true;
				console.log(
					"✓ Updated esbuild.config.mjs: added entry point detection (supports both src/main.ts and main.ts)"
				);
			} else if (
				hasEntryPointVar &&
				/entryPoints:\s*\[entryPoint\]/.test(content)
			) {
		...
		if (hasProdLogic) {
			...
			content = content.replace(
				/const\s+prod\s*=.*process\.argv[^;]+;\s*\n?/g,
				""
			);
			// Remove dist/ directory creation logic
			content = content.replace(
				/if\s*\(prod\s*&&\s*!existsSync\(["']dist["']\)\)\s*\{[^}]+\}\s*\n?/g,
				""
			);
			// Remove production-specific messages
			content = content.replace(
				/if\s*\(prod\)\s*\{[^}]+\}\s*else\s*\{[^}]+\}\s*\n?/g,
				""
			);
			updated = true;
			console.log(
				"✓ Updated esbuild.config.mjs: removed production/development split (always outputs to root)"
			);
		}
		...
		if (hasDistOutput || /outfile:\s*outfile/.test(content)) {
			...
			updated = true;
			console.log(
				"✓ Updated esbuild.config.mjs: fixed outfile to always output to root"
			);
		} else if (!/outfile:/.test(content)) {
			// Add outfile if it doesn't exist
			const contextConfigMatch = content.match(
				/(esbuild\.context\s*\(\s*\{[^}]*?)(\n\s*\})/s
			);
			if (contextConfigMatch) {
				const indent =
					contextConfigMatch[1].match(/(\n\s+)$/)?.[1] || "\n\t";
				content = content.replace(
					/(esbuild\.context\s*\(\s*\{[^}]*?)(\n\s*\})/s,
				...
				updated = true;
				console.log(
					"✓ Updated esbuild.config.mjs: added outfile to esbuild.context"
				);
			}
		}
		...
		const hasProductionInCheck =
			/isOneTimeBuild.*production/.test(content) ||
			/args\.includes\(["']production["']\)/.test(content);
		const hasIsOneTimeBuild = /const\s+isOneTimeBuild\s*=/.test(content);
		const hasArgsSlice =
			/const\s+args\s*=\s*process\.argv\.slice\(2\)/.test(content);
		...
		if (hasIsOneTimeBuild && !hasProductionInCheck) {
			...
			for (let i = 0; i < lines.length; i++) {
				...
				if (
					/const\s+isOneTimeBuild\s*=/.test(line) &&
					/args\.includes\(["']build["']\)/.test(line) &&
				...
				// If this is the isOneTimeBuild line but the check is on the next line
					...
					if (
						/args\.includes\(["']build["']\)/.test(nextLine) &&
						!/production/.test(nextLine)
					) {
			...
			if (fixed) {
				content = lines.join("\n");
				console.log(
					'✓ Updated esbuild.config.mjs: build mode detection now supports both "build" and "production" arguments'
				);
			}
		} else if (!hasIsOneTimeBuild && !hasArgsSlice) {
			// Add build mode detection if it doesn't exist
			const watchModeMatch = content.match(
				/(await\s+context\.watch\(\))/
			);
			if (watchModeMatch) {
				...
				updated = true;
				console.log(
					'✓ Updated esbuild.config.mjs: added build mode detection (supports both "build" and "production")'
				);
			}
		}
		...
		console.log(
			"⚠ Warning: Could not update esbuild.config.mjs:",
			error.message
		);
		return false;
	}
...
function setupESLint() {
	...
	if (majorVersion < 16) {
		console.error(
			`❌ Error: Node.js v16+ is required (found ${nodeVersion})`
		);
	...
	try {
		...
		const currentTsVersion =
			packageJson.devDependencies.typescript ||
			packageJson.dependencies?.typescript;
		if (
			currentTsVersion &&
			!isVersionCompatible(currentTsVersion, MIN_TYPESCRIPT_VERSION)
		) {
			console.log(
				`⚠ TypeScript version ${currentTsVersion} is not compatible with ESLint (requires >=4.8.4)`
			);
		...
		if (packageJson.devDependencies?.["builtin-modules"]) {
			...
			updated = true;
			console.log(
				"✓ Removed deprecated builtin-modules package (use module.builtinModules instead)"
			);
		}
		...
		for (const [script, command] of Object.entries(ESLINT_SCRIPTS)) {
			...
			if (currentCommand && currentCommand.includes("--ext")) {
				console.log(
					`✓ Updating ${script} script: removing --ext flag (not needed in ESLint 9)`
				);
				updated = true;
			}
		...
		if (existsSync(eslintrcPath)) {
			try {
				...
				console.log(
					"✓ Removed legacy .eslintrc file (migrated to eslint.config.mjs)"
				);
			} catch {
		...
		if (existsSync(eslintrcJsonPath)) {
			try {
				...
				console.log(
					"✓ Removed legacy .eslintrc.json file (migrated to eslint.config.mjs)"
				);
			} catch {
		...
		if (
			...
			if (migratingFromEslint8) {
				console.log(
					"✓ Successfully migrated from ESLint 8 to ESLint 9"
				);
			}
			console.log("\nNext steps:");
